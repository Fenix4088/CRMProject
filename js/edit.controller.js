const editController = ((ctrlModel, ctrlEditView) => {
  ...
  document
    .querySelector(editDOMElements.saveBtn)
    .addEventListener("click", changingRequest);
  ...
  function collectNewValues(inputs) {
    ...
    const courseSelectType =
      inputs.courseSelect.options[inputs.courseSelect.options.selectedIndex]
        .value;
    const courseSelectName =
      inputs.courseSelect.options[inputs.courseSelect.options.selectedIndex]
        .innerText;
